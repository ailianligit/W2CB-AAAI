...
class DatasetDistance():
    ...
    def get_label_distance(self, maxsamples=10000):
        ...
        extracted_elements = W[indices, indices + d // 2]
    ...
    def dual_sol(self, maxsamples=10000, return_coupling=False):
        ...
        if self.X1 is None or self.X2 is None:
            if (not self.method == 'jdot') and (self.λ_y is not None) and (self.λ_y > 0):
                s = time()
                print("WROGNG: unexpected line 659")
                _ = self._get_label_distances()
                logger.info('/* Time to precompute label distances: {} */'.format(time() - s))
                print(f"Finished first label distance in: {time() - s} s.")
            dat_t = time()
            print("load datasets again?")
            self._load_datasets(maxsamples, device=device_dists)
...
#         first_dist = []
    ...
    def compute_coupling(self, entreg=None, gpu=None, **kwargs):
        ...
        embedding = MDS(n_components=10, n_init=10,
                        max_iter=10000, dissimilarity='precomputed')
        mds = embedding.fit_transform(W)
        z1_labels = torch.stack([torch.from_numpy(mds[target])
                                for target in self.Y1], dim=0).squeeze(1).to(device)
        z2_labels = torch.stack([torch.from_numpy(mds[target])
                                for target in self.Y2], dim=0).squeeze(1).to(device)
        new_z1 = torch.cat((self.X1, z1_labels), dim=1)
        new_z2 = torch.cat((self.X2, z2_labels), dim=1)
